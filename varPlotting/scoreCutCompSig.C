...
void scoreCutCompSig(bool etaSplit,bool diphotonCuts){
    gROOT->Reset();
    gStyle->SetPalette(1);
    ...
    string gghLabelIDMVA = "GGH W/ idMVA > -0.99";
    ...
    string vbfLabelIDMVA = "100*VBF W/ idMVA > -0.99";
    ...
    string tthLabelIDMVA = "500*TTH W/ idMVA > -0.99";
    ...
    string bbhLabelIDMVA = "500*HHTo2B2G W/ idMVA > -0.99";
    ...
    string allLabelsIDMVA[4] = {gghLabelIDMVA,vbfLabelIDMVA,tthLabelIDMVA,bbhLabelIDMVA};
    
    if(etaSplit == true) dirStr += "EtaSplit/";
    
    string outNameGen = dirStr + "Signal_idMVACut";
    ...
    for (int e = 0; e < nEta; e++){
        ...
        string cutStringLeadLooseGen,cutStringSubLooseGen,cutStringLeadTightGen,cutStringSubTightGen,cutStringLeadLooseMass,cutStringSubLooseMass,cutStringLeadTightMass,cutStringSubTightMass,cutStringLeadIDMVAGen,cutStringSubIDMVAGen,cutStringLeadIDMVAMass,cutStringSubIDMVAMass;
        if(diphotonCuts == true){
            cutStringLeadLooseGen = "(leadGenMatchType == 1" + massCutSig + etaCuts[e] + kinCutLeadLoose + kinCutSubLoose + ")*weight";
            cutStringSubLooseGen = "(subGenMatchType == 1" + massCutSig + etaCuts[e] + kinCutSubLoose + kinCutLeadLoose + ")*weight";

            cutStringLeadTightGen = "(leadGenMatchType == 1" + massCutSig + etaCuts[e] + kinCutLeadTight + preselLead + kinCutSubTight + preselSub + ")*weight";
            cutStringSubTightGen = "(subGenMatchType == 1" + massCutSig + etaCuts[e] + kinCutSubTight + preselSub + kinCutLeadTight + preselLead + ")*weight";
            
            cutStringLeadIDMVAGen = "(leadGenMatchType == 1" + massCutSig + etaCuts[e] + kinCutLeadLoose + kinCutSubLoose + idMVACutLead + idMVACutSub +")*weight";
            cutStringSubIDMVAGen = "(subGenMatchType == 1" + massCutSig + etaCuts[e] + kinCutSubLoose + kinCutLeadLoose + idMVACutSub + idMVACutLead + ")*weight";
            
            cutStringLeadLooseMass = "(leadGenMatchType == 1" + massCutLoose + etaCuts[e] + kinCutLeadLoose + kinCutSubLoose + ")*weight";
            cutStringSubLooseMass = "(subGenMatchType == 1" + massCutLoose + etaCuts[e] + kinCutSubLoose + kinCutLeadLoose + ")*weight";
            
            cutStringLeadTightMass = "(leadGenMatchType == 1" + massCutLoose + etaCuts[e] + kinCutLeadTight + preselLead + kinCutSubTight + preselSub + ")*weight";
            cutStringSubTightMass = "(subGenMatchType == 1" + massCutLoose + etaCuts[e] + kinCutSubTight + preselSub + kinCutLeadTight + preselLead + ")*weight";

            cutStringLeadIDMVAMass = "(leadGenMatchType == 1" + massCutLoose + etaCuts[e] + kinCutLeadLoose + kinCutSubLoose + idMVACutLead + idMVACutSub + ")*weight";
            cutStringSubIDMVAMass = "(subGenMatchType == 1" + massCutLoose + etaCuts[e] + kinCutSubLoose + kinCutLeadLoose + idMVACutSub + idMVACutLead + ")*weight";
        }
        if(diphotonCuts == false){
            cutStringLeadLooseGen = "(leadGenMatchType == 1" + massCutSig + etaCuts[e] + kinCutLeadLoose + ")*weight";
            cutStringSubLooseGen = "(subGenMatchType == 1" + massCutSig + etaCuts[e] + kinCutSubLoose + ")*weight";
            
            cutStringLeadTightGen = "(leadGenMatchType == 1" + massCutSig + etaCuts[e] + kinCutLeadTight + preselLead + ")*weight";
            cutStringSubTightGen = "(subGenMatchType == 1" + massCutSig + etaCuts[e] + kinCutSubTight + preselSub + ")*weight";
            
            cutStringLeadIDMVAGen = "(leadGenMatchType == 1" + massCutSig + etaCuts[e] + kinCutLeadLoose + idMVACutLead + ")*weight";
            cutStringSubIDMVAGen = "(subGenMatchType == 1" + massCutSig + etaCuts[e] + kinCutSubLoose + idMVACutSub + ")*weight";
            
            cutStringLeadLooseMass = "(leadGenMatchType == 1" + massCutLoose + etaCuts[e] + kinCutLeadLoose + ")*weight";
            cutStringSubLooseMass = "(subGenMatchType == 1" + massCutLoose + etaCuts[e] + kinCutSubLoose + ")*weight";
            
            cutStringLeadTightMass = "(leadGenMatchType == 1" + massCutLoose + etaCuts[e] + kinCutLeadTight + preselLead + ")*weight";
            cutStringSubTightMass = "(subGenMatchType == 1" + massCutLoose + etaCuts[e] + kinCutSubTight + preselSub + ")*weight";
            
            cutStringLeadIDMVAMass = "(leadGenMatchType == 1" + massCutLoose + etaCuts[e] + kinCutLeadLoose + idMVACutLead + ")*weight";
            cutStringSubIDMVAMass = "(subGenMatchType == 1" + massCutLoose + etaCuts[e] + kinCutSubLoose + idMVACutSub + ")*weight";
        }
        ...
        for(int i = 0; i < nVars; i++){
            ...
            if(varNames[i] == "hggMass"){
                ...
                cutStringLeadIDMVA = cutStringLeadIDMVAMass;
                cutStringSubIDMVA = cutStringSubIDMVAMass;
                ...
                cutStringLeadIDMVA = cutStringLeadIDMVAGen;
                cutStringSubIDMVA = cutStringSubIDMVAGen;
            ...
            for(int j = 0; j < nTrees; j++){
                ...
                string cutStringLeadIDMVAInside = cutStringLeadIDMVA;
                
                string cutStringSubLooseInside = cutStringSubLoose;
                string cutStringSubTightInside = cutStringSubTight;
                string cutStringSubIDMVAInside = cutStringSubIDMVA;
                ...
                if(j == 2){
                    ...
                    cutStringLeadIDMVAInside += "*500";

                    cutStringSubLooseInside += "*500";
                    cutStringSubTightInside += "*500";
                    cutStringSubIDMVAInside += "*500";
                }
                if(j == 3){
                    cutStringLeadLooseInside += "*500";
                    cutStringLeadTightInside += "*500";
